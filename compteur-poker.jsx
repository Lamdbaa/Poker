...
function computePots(ps) {
  const levels = [...new Set(ps.filter((p) => p.committed > 0).map((p) => p.committed))].sort((a, b) => a - b);
  let prev = 0;
  const raw = [];
  for (const lvl of levels) {
    ...
    for (const p of ps) {
      ...
      if (!p.folded && p.committed >= lvl) eligible.push(p.id);
    }
  ...
  for (const pot of raw) {
    ...
    const same =
      last && last.eligible.length === pot.eligible.length &&
      last.eligible.every((id, i) => id === pot.eligible[i]);
    if (last && (same || pot.eligible.length === 0)) last.amount += pot.amount;
    else merged.push({ ...pot });
  }
  const fallback = seated(ps).map((p) => p.id);
  return merged.map((p, i) => {
    const eligible = p.eligible.length ? p.eligible : fallback;
    return { amount: p.amount, eligible, key: i, winners: eligible.length === 1 ? [...eligible] : [] };
  });
}
...
function Hand({ g, push, raising, setRaising, undo, canUndo }) {
  ...
  const toCall = Math.min(g.currentBet, me.stack);
    ...
    <>
      <Felt>
        ...
        <div style={{ color: T.muted, fontSize: 14 }}>
          jetons dans le pot{g.currentBet > 0 ? ` · ${g.currentBet} pour suivre` : " · personne n'a misé"}
        </div>
  ...
  return (
    <>
      <Felt tight>
        ...
        <div style={{ color: T.muted, fontSize: 14 }}>
          {total} jetons à distribuer{pots.length > 1 ? ` en ${pots.length} pots` : ""}
        </div>
      </Felt>
          ...
          {pots.map((pot, k) => (
            <Panel key={pot.key}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <span style={{ fontSize: 14.5, color: T.muted }}>
                  {pots.length === 1 ? "Le pot" : k === 0 ? "Pot principal" : `Pot secondaire ${k}`}
                </span>
